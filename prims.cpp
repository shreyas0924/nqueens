...
std::map<int, std::tuple<float, float, float>> nodeColors;
...
void drawpoint()
{
     ...
     {
          // Check if the node color is already generated
          if (nodeColors.find(i) == nodeColors.end())
          {
               ...
               // Store the color for the node index
               nodeColors[i] = std::make_tuple(red, green, blue);
          }

          // Retrieve the color for the node index
          float red, green, blue;
          std::tie(red, green, blue) = nodeColors[i];
...
void frontpage()
{
     ...
     // Line to display the personal details
     ...
     glColor3f(0.9, 0.8, 0.9);
     bitmap_output(width / 3 + 150 - (glutBitmapLength(GLUT_BITMAP_HELVETICA_18, (const unsigned char *)"Shubhankar Sharma       1JS20CS158") / 2), height - height / 1.7, "Shubhankar Sharma       1JS20CS158", GLUT_BITMAP_HELVETICA_18);

     glColor3f(0.9, 0.8, 0.9);
     bitmap_output(width / 3 + 150 - (glutBitmapLength(GLUT_BITMAP_HELVETICA_18, (const unsigned char *)"Srishti Koul                    1JS20CS164") / 2), height - height / 1.5, "Srishti Koul                    1JS20CS164", GLUT_BITMAP_HELVETICA_18);
     ...
     bitmap_output(width / 3 + 150 - (glutBitmapLength(GLUT_BITMAP_HELVETICA_18, (const unsigned char *)"Mrs. Rashmi B N") / 2), 50, "Mrs Rashmi B N", GLUT_BITMAP_HELVETICA_18);
...
void input()
{
     ...
     {
          // Generate a random cost from 0 to 999
          cost = rand() % 100;
          // The modulus 1000 restricts the range from 0 to 999
